...
class VolumeQueryProvider(Protocol):
    def get_volume_query(self)->Composed:...
...
class MiovisionDBVolumeProvider:
    def __init__(self, connection_string: ConnectionStringProvider, query_provider: MiovisionDBQueryProvider) -> None:
        ...
        self._query = query_provider.get_volume_query()
    ...
    def _get_miovision_id(self, file_stem:str)->int:
        ...
        return int(study_type_id[-1])
    
    def return_volumes(self, file_path: Path)->dict[str,DirectionalVolumeAttr]:
        miovision_id = self._get_miovision_id(file_path.stem)
        self._cursor.execute(self._query,[miovision_id])
